...
const RegistrationPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (invitationCode) {
        const inviteCheckResponse = await axios.post(
          "invites/check-invite-code",
          { uniqueInvitationCode: invitationCode }
        );
        if (!inviteCheckResponse.data.valid) {
      ...
      // Create user
      await axios.post("/user/details/signup", {
        name,
        email: lowerCaseEmail,
        phoneNumber: formattedPhoneNumber,
        password,
        enteredInvitationCode: invitationCode,
      });
      ...
      <div className="w-full max-w-md">
        <div className="bg-white shadow-xl rounded-lg p-6">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div>
              <label htmlFor="invitationCode" className="block text-sm space-micro font-medium text-gray-700">
          ...
          <p className="mt-4 text-center text-sm space-micro text-gray-600">
            Already have an account?{" "}
            <a href="/" className="text-indigo-600 space-micro hover:underline">
              Sign in
            </a>
          </p>
        </div>
